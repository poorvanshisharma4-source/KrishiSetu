  ...
  useEffect(() => {
    const loadAnalytics = async () => {
      try {
        ...
        const analyticsResponse = await api.post('/ai/buyer-analytics', {
          buyerId,
          targetCrops: ['wheat', 'rice', 'tomatoes'],
        })

        setAiResponse(analyticsResponse?.data ?? analyticsResponse)
        setError(null)
      } catch (err: any) {
        console.error('Buyer analytics fetch failed:', err)
        setError(err?.message || 'Unable to load buyer analytics data')
      } finally {
        setLoading(false)
      }
    }
    ...
    <div className="min-h-screen bg-[#F8F6F0] text-black">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 rounded-2xl bg-white p-6 border border-gray-200/60 shadow-sm">
          ...
          <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-3">
            {priceForecasts.map((forecast, idx) => (
              <div key={idx} className="rounded-xl border border-gray-200/60 p-5 bg-gray-50/20 space-y-4">
                <h3 className="font-extrabold text-gray-900 text-base border-b border-gray-100 pb-2">
                  {forecast.commodity}
                </h3>
                <div className="space-y-3">
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Current Price</p>
                    <p className="text-lg font-black text-gray-900">
                      ₹{forecast.current}/kg
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">AI Forecasted Price</p>
                  ...
                  <div className="rounded-xl bg-gray-50 border border-gray-100 p-2.5 text-center">
                    <p
                      className={`text-xs font-bold ${
                        forecast.change >= 0 ? 'text-red-600' : 'text-emerald-600'
                      }`}
                    >
                      {forecast.change >= 0 ? '▲ +' : '▼ '}
          ...
          {recommendations.map((rec, idx) => (
            ...
            >
              <div className="mb-6 flex items-center gap-3">
                ...
                <h2 className="text-lg font-black text-gray-900 tracking-tight">{rec.title}</h2>
              </div>
              <ul className="space-y-3.5">
                {rec.items.map((item, itemIdx) => (
                  <li key={itemIdx} className="flex gap-3 text-gray-700 items-start">
                    <span className="mt-1.5 h-2.5 w-2.5 rounded-full bg-emerald-600 shrink-0" />
                    <span className="text-sm font-medium leading-relaxed text-gray-600">{item}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="mt-8 rounded-2xl bg-white p-6 border border-gray-200/60 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-gray-900">AI Analytics Results</h2>
              <p className="text-sm text-gray-500">Loaded from the buyer analytics endpoint.</p>
            </div>
          </div>

          {loading ? (
            <div className="rounded-2xl bg-gray-50 p-6 text-center text-gray-600">Loading AI analytics...</div>
          ) : error ? (
          ...
          {recommendations.map((rec, idx) => (
            ...
            >
              ...
              <ul className="space-y-3.5">
                {rec.items.map((item, itemIdx) => (
                  <li key={itemIdx} className="flex gap-3 text-gray-700 items-start">
                    <span className="mt-1.5 size-1.5 flex-shrink-0 rounded-full bg-emerald-600" />
                    <span className="text-sm font-medium leading-relaxed text-gray-600">{item}</span>
                  </li>
                ))}
              </ul>

...
import React from 'react';
import { useRouter, usePathname } from 'next/navigation';
import { LayoutDashboard, FileText, Users, Plus, Settings, LogOut, BarChart3, MessageSquare, Package } from 'lucide-react';
...
export default function BuyerRootLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#F8F6F0] text-black">
      {/* 🟢 Main Global Navbar ko yahan mount kiya taaki Home, About, FAQ sab uniform dikhe */}
      <Navbar />
                ...
                {sidebarItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => router.push(item.path)}
                      className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-all text-left ${
                        isActive ? 'bg-[#D97706] text-white' : 'text-gray-600 hover:bg-gray-100'
                      }`}
                    >
                      <item.icon className="w-5 h-5" />
                      <span className="font-medium">{item.label}</span>

  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (res.data && res.data.success && res.data.data && res.data.data.length > 0) {
        // Map backend response structure to component UI structure
        const mappedData: NotificationItem[] = res.data.data.map((item: any) => ({
          _id: item._id,
          title: item.title || "New Notification",
          message: item.message || "",
          unread: item.isRead === false,
          time: new Date(item.createdAt || Date.now()).toLocaleDateString("en-IN", {
            day: 'numeric',
            month: 'short',
            hour: '2-digit',
            minute: '2-digit'
        ...
        // Use fallback if no database notifications exist yet

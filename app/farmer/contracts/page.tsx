...
export default function FarmerContractsPage() {
  ...
  const fetchContracts = async () => {
    try {
      setLoading(true);
      setError('');
      // Using your custom native api fetch client
  ...
  const filteredContracts = contracts.filter((c) => {
    const crop = c.cropName || c.requirement?.cropName || '';
    const buyerName = c.buyer?.name || '';
    const matchesSearch =
  ...
  {t('farmerContracts.description')}
      ...
      {/* Filters & Search */}
      <div className="bg-white p-4 rounded-xl shadow-sm mb-6 flex flex-col sm:flex-row gap-4 items-center justify-between">
  ...
  {t('farmerContracts.adjustFilter')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredContracts.map((contract) => {
            const cropName = contract.cropName || contract.requirement?.cropName || 'Crop Contract';

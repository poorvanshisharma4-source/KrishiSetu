...
const mapRequirement = (item: any): RequirementCard => ({
  ...
  qualityGrade: item.grade || 'Standard',
...
export default function BuyerMarketplace() {
  ...
  const handleSendProposal = async (requirementId: string) => {
  try {
    await api.post('/requests', {
      requirementId,
      message: 'Interested in supplying this crop.',
    })
                ...
                {filteredRequirements.map((req) => (
                  ...
                  >
                    ...
                    <div className="border-t border-green-50 p-4">
                      <button
                        onClick={() =>
  handleSendProposal(req.requirementId)
}

...
import {
  useAICopilot,
} from '@/components/ai-copilot/AICopilotContext'
import { useLanguage } from '@/components/LanguageContext'

const navLinks = [
  { key: 'navbar.home', label: 'Home', href: '/' },
  { key: 'navbar.about', label: 'About', href: '/about' },
  { key: 'navbar.features', label: 'Features', href: '/features' },
  { key: 'navbar.dashboard', label: 'Dashboard', href: '/dashboard' },
  { key: 'navbar.faq', label: 'FAQ', href: '/faq' },
  { key: 'navbar.contact', label: 'Contact', href: '/contact' },
]

const languages = [
  { label: 'English', value: 'en' as const },
  { label: 'हिन्दी', value: 'hi' as const },
]
...
export function Navbar({ onCopilotClick }: NavbarProps = {}) {
  ...
  const isFarmerAuth =
    pathname === '/farmer/login' || pathname === '/farmer/signup'

  // Dashboard areas dynamic checks
  const isBuyerDashboard =
    pathname?.startsWith('/buyer/') && !isBuyerAuth

  const isFarmerDashboard =
    pathname?.startsWith('/farmer/') && !isFarmerAuth

  const activeLanguage =
    languages.find((lang) => lang.value === language)?.label || 'English'
            ...
            <div className="flex items-center space-x-4">
              <Link href="/buyer/post-requirement">
                <Button
                  variant="default"
                  size="sm"
                  className="bg-amber-600 hover:bg-amber-700 h-9 rounded-xl font-bold"
                >
        ...
        {isFarmerDashboard && (
          <>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3 pl-4 border-l border-gray-200">
                <Image
                  src="https://images.pexels.com/photos/235721/pexels-photo-235721.jpeg?auto=compress&cs=tinysrgb&w=100"
                  alt="Farmer"
                  width={36}
                  height={36}
        ...
        {!isBuyerDashboard && !isFarmerDashboard && (
          <>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="hidden sm:block">
                <AICopilotToggle
                  onClick={onCopilotClick || openCopilot}
                />
              </div>
